...
neetcode : https://www.youtube.com/watch?v=LQ8TuG_QADM
```java
class Solution {
...
class Solution {
    ...
    public int findWaves(int num){
        ...
        int prev = 0;
        int curr = 1;
        while(curr < n.length() -1 ){
            if(n.charAt(prev) > n.charAt(curr)){
                prev++;
                curr++;
                if(n.charAt(prev) < n.charAt(curr)){
                    waves++;
            ...
            else if(n.charAt(prev) < n.charAt(curr)){
                prev++;
                curr++;
                if(n.charAt(prev) > n.charAt(curr)){
                    waves++;
...
class Solution {
    ...
    long[] helper(String s, int idx, boolean isBound, int twoBefore, int oneBefore, boolean leadingZero){
        if(idx == s.length()){
